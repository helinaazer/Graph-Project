...
private:
  ...
  int numOfEdges;
  //map of string and vertex pointer
  map<string, Vertex*> _vertexMap;
  // depth-first search helper function, starts from current label
  void dfsHelper(const string &currentLabel, set<string> &visited,
                 void visit(const string &label));
  // helper function for dijkstra's algorithm
  void dijkstraHelper(const string &currentLabel, int currentWeight, set<string> &visited, map<string, int> &weights, map<string, string> &previous) const;
  // function that gets the minimum edge weight. Return false if no edge found
  bool getMinimumWeightEdge(vector<Edge *> &edges, size_t &minIndex,
                                   set<string> &visited) const;
  // function to compare edges. Used to print out the edges in alphabetical
  // order
  static bool compareEdges(Edge *first, Edge *second);
  };
#endif //GRAPH_H